$(document).ready(function () {
   ...
   function getSearchInput(sectionId) {
      ...
      console.log("SearchInputSelector is : " + searchInputSelector);

      let searchInput = $(searchInputSelector).val();
      if (searchInput == null || isNaN(searchInput) || searchInput == "") {
         showStudentNotFoundOrNull("Search field cannot be Null or NaN");
         return;
      }
   ...
   function sendStudentDataToBackend(formData, method, sectionId, stuid) {
      ...
      $.ajax({
         ...
         error: function (xhr, status, error) {
            ...
            if (method == "POST") {
               showStudentNotFoundOrNull(
                  "Oops! Connection issue. Unable to add student information. Please try again later."
               );
            } else {
               showStudentNotFoundOrNull(
                  "Oops! It seems like there's a connection issue. Please try again later."
               );
            }
         },
         complete: function () {
   ...
   function showStudentNotFoundOrNull(
      ...
      // Schedule hiding the element after 10 seconds
      setTimeout(function () {
         hideStudentNotFoundOrNull();
